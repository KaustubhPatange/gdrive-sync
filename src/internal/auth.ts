...
import { type Config, parseConfig, saveConfig } from './config';
import log from './log';
...
async function authenticateService(serviceAccountJson: string) {
	try {
		...
		log.error(
			'Service account authentication failed. Check your SERVICE_ACCOUNT_JSON.',
		);
		throw error;
	}
}
	...
	try {
		...
		if (tokens?.refresh_token) {
			...
			try {
				...
				log.warning(
					'Saved tokens are invalid or expired. Starting new authentication flow.',
				);
			}
		}
...
export async function authenticate(configPath: string) {
	...
	if (config.serviceAccount && !config.loginType) {
	...
	throw new Error(
		'Invalid configuration. Please provide either serviceAccount or oauth credentials.',
	);
}
