...
import { downloadFile, extractFileIdFromUrl, getDriveFolderId, verifyFolderExists, createFolder, checkFileExists, isFolder, checkFolderExists } from './internal/drive'
import { calculateDirectoryHashes, parseHashFile, hashMapToString, findChangedFiles } from './internal/hash';
 ...
 */
async function uploadFile(drive: any, filePath: string, folderId: string, forceOverwrite: boolean = false): Promise<{id?: string; name?: string, upload: boolean}> {
  ...
  if (fs.statSync(filePath).isDirectory()) {
    log.error(`${filePath} is a directory. Use the upload-folder command instead.`);
    return {upload: false};
  }
  ...
  if (existingFileId && !forceOverwrite) {
    log.warning(`File "${fileName}" already exists in the target folder. Use --force to overwrite.`);
  ...
  try {
    ...
    if (existingFileId && forceOverwrite) {
      ...
      progressBar.stop();
      log.success(`File updated successfully: ${fileName} (ID: ${response.data.id})`);
    } else {
      const fileMetadata = {
        name: fileName,
        parents: [folderId],
 ...
 */
async function uploadFolder(drive: any, folderPath: string, targetFolderId: string, trackChanges: boolean = false, forceOverwrite: boolean = false) {
  ...
  if (trackChanges) {
    ...
    if (remoteHashes.exists) {
      ...
      const { changed, added, unchanged } = findChangedFiles(localHashes, remoteHashes.hashes);
      
      log.info(`Found ${changed.length} changed files, ${added.length} new files, ${unchanged.length} unchanged files.`);
      ...
      log.info('No hash file found on Google Drive. Will upload all files and create a new hash file.');
    ...
    const finalUploadedHashes = remoteHashes.exists ? remoteHashes.hashes : localHashes;

    // Upload only the files that need to be uploaded
    for (const filePath of filesToUpload) {
      const dirPath = path.dirname(filePath);
      
      // Ensure the directory structure exists in Drive
      ...
      if (dirPath && dirPath !== '.') {
        ...
        for (const part of pathParts) {
          ...
          if (!folderMap.has(newFolderPath)) {
            ...
            const folderExists = await checkFolderExists(drive, part, currentFolderId);
            ...
            if (folderExists) {
              ...
              const newFolder = await createFolder(drive, part, currentFolderId);
      ...
      try {
        const result = await uploadFile(drive, path.join(folderPath, filePath), currentFolderId, forceOverwrite);
        if (result.upload) {
    ...
    // Upload the hash file after all files have been processed, but only if there were successful uploads
    await uploadHashFile(drive, targetFolderId, hashMapToString(finalUploadedHashes), remoteHashFileId);
  } else {
    // Count files recursively for normal upload
    ...
    log.info(`Found ${totalFiles} files to upload.`);
    // Process folder recursively for normal upload
    const processFolder = async (localFolderPath: string, parentFolderId: string) => {
      ...
      for (const item of items) {
        ...
        if (stats.isFile()) {
          try {
            const result = await uploadFile(drive, itemPath, parentFolderId, forceOverwrite);
            if (result.upload) {
              uploadedFiles++;
            }
          } catch (error) {
            log.error(`Failed to upload ${itemPath}: ${(error as Error).message}`);
          }
        } else if (stats.isDirectory()) {
          // Create subfolder in Drive
          const subfolderResponse = await createFolder(drive, item, parentFolderId);
  ...
  log.success(`Folder upload complete. Uploaded ${uploadedFiles}/${totalFiles} files.`);
  return {
    folderName,
    targetFolderId,
    totalFiles,
 ...
 */
async function getHashFileFromDrive(drive: any, folderId: string): Promise<{ exists: boolean, hashes: Map<string, string>, fileId?: string }> {
 ...
 */
async function uploadHashFile(drive: any, folderId: string, hashContent: string, existingFileId?: string): Promise<void> {
  log.process('Uploading hash file to Google Drive...');
  
  // Write hash content to a temporary file
 ...
 */
async function listFolderContents(drive: any, folderId: string, relativePath: string = ''): Promise<Array<{id: string, name: string, path: string}>> {
  const res = await drive.files.list({
 ...
 */
async function downloadFolder(drive: any, folderId: string, outputPath: string) {
  ...
  const downloadPromises = files.map(async (file) => {
    ...
    try {
      ...
      const isGoogleDoc = fileMetadata.data.mimeType.includes('application/vnd.google-apps');
      
      if (isGoogleDoc) {
        // For Google Docs, we need to pass the directory path, not the full file path
        await downloadFile(drive, file.id, dirPath);
      } else {
        // For regular files, we can pass the full path
  ...
  if (failed > 0) {
    ...
    results.filter(r => !r.success).forEach(result => {
      log.error(`Failed to download ${result.file.name}: ${(result.error as Error).message}`);
    });
  }
...
function formatErrorDetails(error: any): string {
  ...
  if (error.code === 'ENOENT') {
    return `File System Error: File or directory not found - ${error.path || ''}`;
  }
  
  if (error.code === 'EACCES') {
    return `File System Error: Permission denied - ${error.path || ''}`;
  }
  ...
  try {
    ...
    addAuthOption(
      program
        ...
        .option('-f, --folder <target>', 'Target folder ID or URL in Google Drive')
        .option('-t, --track', 'Track file changes and only upload modified files', false)
        .option('--force', 'Overwrite existing files with the same name', false)
      ...
      try {
        ...
        if (options.folder.includes('drive.google.com') || options.folder.includes('docs.google.com')) {
        ...
        if (isDirectory) {
          ...
          await uploadFolder(drive, sourcePath, targetFolderId, options.track, options.force);
        } else {
          if (options.track) {
            log.warning('Change tracking is only available for folder uploads. Ignoring --track option.');
          }
    ...
    addAuthOption(
      program
        ...
        .argument('<fileIdOrUrl>', 'ID or URL of the file or folder to download')
      ...
      try {
        ...
        if (isDirectory) {
          log.info('The provided ID is a folder. Downloading folder contents...');
        ...
        if (error.response && error.response.status === 404) {
          log.error('The file or folder does not exist or you do not have permission to access it.');
        }
      }
    });
    ...
    addAuthOption(
      ...
      try {
        ...
        log.error(`Error retrieving file information: ${formatErrorDetails(error)}`);
      }
    });
